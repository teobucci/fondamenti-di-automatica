...
\begin{equation*}
\mathrm{det} A=\prod ^{n}_{i=1} \lambda _i
\end{equation*}
\end{ricalg}
Quindi:
\begin{itemize}
\item tempo continuo: $\mathrm{det} A=0\Leftrightarrow \exists \lambda _i =0$ (se ne esiste almeno uno nullo)
\item tempo discreto: $\mathrm{det}( I-A) =0\Leftrightarrow \exists \lambda _i =1$ (se ne esiste almeno uno unitario)
\end{itemize}
...
\begin{equation}
\boxed{\begin{aligned}
{\displaystyle x(t)} & {\displaystyle =e^{At} x( 0) +\int\limits ^{t}_0 e^{A( t-\tau )} bu( \tau ) d\tau }\\
...
\begin{enumerate}
\item $A$ è asint. stabile $\Leftrightarrow $ $\mathrm{Re}\left( \lambda _i\right) < 0\ \forall i$
\item $A$ è sempl. stabile $\Leftrightarrow $ $\mathrm{Re}\left( \lambda _i\right) \leqslant 0\ \forall i,\ \exists i:\mathrm{Re}\left( \lambda _i\right) =0$
\item $A$ è instabile $\Leftarrow $ $\exists i:\mathrm{Re}\left( \lambda _i\right)  >0$
\end{enumerate}

Gli autovalori sono le radici del polinomio caratteristico
\begin{equation*}
\Delta _A\left( \lambda \right) =\Delta _{\tilde{A}}\left( \lambda \right) =\mathrm{det}\left( \lambda I-A\right) =\lambda ^{n} +\alpha _1 \lambda ^{n-1} +\cdots +\alpha _n
\end{equation*}
...
\begin{aligned}
\Delta _A\left( \lambda \right) =\mathrm{det}\left( \lambda I-A\right) & =\lambda ^{n} +\alpha _1 \lambda ^{n-1} +\cdots +\alpha _n\\
 & =\left( \lambda -\lambda _1\right) \cdots \left( \lambda -\lambda _n\right)\\
 & =\prod\nolimits ^{n}_{i=1}\left( \lambda -\lambda _i\right)\\
 & =\prod\nolimits ^{k}_{i=1}\left( \lambda -\lambda _i\right)^{a_i}
\end{aligned}
\end{equation*}
...
\end{bmatrix}
\end{equation*}
A cosa ci è servito questo? Se la matrice è diagonalizzabile riusciamo a decomporre $z$ in $n$ sottosistemi tra loro indipendenti, se non lo è posso fare questa decomposizione in sistemi che tra loro non interagiscono, quindi si tratta di capire come funziona il singolo $J^{l}_i$: la sua micro-equazione di stato è fatta così
\begin{equation*}
\dot{z}^{l}_i =J^{l}_i z^{l}_i
\end{equation*}
applicando la formula di Lagrange a questo diventa
\begin{equation*}
z^{l}_i\left( t\right) =e^{J^{l}_i t} z^{l}_i\left( 0\right)
\end{equation*}
Si tratta ora di capire com'è fatta $e^{J^{l}_i t}$, si può dimostrare che è fatta così
\begin{equation*}
e^{J^{l}_i t} =\begin{bmatrix}
1 & t & t^{2} /2 & \dotsc  & \\
 & 1 & t & \ddots  & \vdots \\
 &  & 1 & \ddots  & t^{2} /2\\
 &  &  & \ddots  & t\\
...
Sia $\dot{x} =Ax$, allora
\begin{itemize}
\item $\mathrm{Re}\left( \lambda _i\right) < 0\ \forall i\Leftrightarrow A$ asint. stabile
\item $\exists i:\mathrm{Re}\left( \lambda _i\right)  >0\Rightarrow A$ instabile (instabilità forte)
\item $\mathrm{Re}\left( \lambda _i\right) \leqslant 0\ \forall i,\exists i:\mathrm{Re}\left( \lambda _i\right) =0$
\begin{itemize}
\item $b_i =1$ (ossia $a_i =n_i$) $\forall i:\mathrm{Re}\left( \lambda _i\right) =0\Leftrightarrow A$ sempl. stabile
\item $b_i  >1$ per almeno un $i$, cioè l'autovalore è irregolare e tale che $\mathrm{Re}\left( \lambda _i\right) =0\Rightarrow A$ instabile (instabilità debole)
\end{itemize}
\end{itemize}
...
\begin{equation*}
\mathrm{tr} A=\sum\limits ^{n}_{i=1} a_{ii} =\sum\limits ^{n}_{i=1} \lambda _i
\end{equation*}
\end{ricalg}

Consideriamo i sistemi a \textit{tempo continuo} $\dot{x} =Ax$.
\begin{itemize}
\item Se $\mathrm{tr} A >0$, ovvero $\left( \lambda _1 +\cdots +\lambda _n  >0\right)$, allora esiste almeno un autovalore positivo $\Rightarrow A$ instabile.
...
\begin{equation*}
\begin{aligned}
\Delta _A\left( \lambda \right) & =\mathrm{det}\left( \lambda I-A\right)\\
...
\begin{equation*}
\boxed{A\ \text{asint. stabile} \ \ \Leftrightarrow \ \ \mathrm{det} D_i  >0\ \forall i=1,\dotsc ,n}
\end{equation*}
...
\begin{cases}
\alpha _1  >0\\
\alpha _2 \alpha _1  >\alpha _3 \ \left(\text{cioè} \ \mathrm{det} D_2  >0\right)\\
\alpha _3\left(\mathrm{det} D_2\right)  >0
...
\begin{equation*}
X_R(t) =\left\{\text{insieme degli} \ \tilde{x} \ \text{raggiungibili in} \ t\right\} =\mathrm{Im}( \psi (t)) \subseteq \mathbb{R}^{n}
\end{equation*}
...
\begin{definition}
$( A,b)$ è completamente raggiungibile se $X_R =\mathbb{R}^{n}$, ovvero $\mathrm{dim} X_R =n$.
\end{definition}
...
Si dimostra che
\begin{equation*}
X_R =\mathrm{Im}( R) =\mathrm{span}( R)
\end{equation*}
da cui si deduce anche
\begin{equation*}
\mathrm{dim}( X_R) =\mathrm{rank}( R)
\end{equation*}
e che
\begin{equation*}
...
\begin{equation*}
X_{NR} =X^{\perp }_R \ \ \Rightarrow \ \ \mathrm{dim} X_{NR} =n-r
\end{equation*}
...
Studiamo il caso di non completa osservabilità, cioè $\mathrm{rank}( O) =o< n$. Definiamo $X_O$ \textbf{sottospazio di osservabilità}
\begin{equation*}
X_O =X^{\perp }_{NO} \ \ \Rightarrow \ \ \mathrm{dim} X_O =\mathrm{rank}( O) =o
\end{equation*}
...
\begin{equation*}
\begin{aligned}
y(t) =cx(t) & =c\int ^{t}_0 e^{A( t-\tau )} bu( \tau ) d\tau \\
...
\begin{aligned}
y(t) =cx(t) & =c\int ^{t}_0 e^{A( t-\tau )} bu( \tau ) d\tau \\
 & =\int ^{t}_0\boxed{ce^{A( t-\tau )} b} u( \tau ) d\tau \\
 & =\int ^{t}_0\boxed{g( t-\tau )} u( \tau ) d\tau 
\end{aligned}
\end{equation*}
...
\begin{aligned}
U( s) & =\int ^{+\infty }_0\mathrm{imp}(t) e^{-st} dt\\
...
U( s) & =\int ^{+\infty }_0 1\cdotp e^{-st} dt\\
 & =-\frac{1}{s}\left[ e^{-st}\right]^{+\infty }_0 =\frac{1}{s} ,\ \ \text{se} \ \mathrm{Re}( s)  >0
\end{aligned}
\end{equation*}
...
h(t) =\int\limits ^{t}_0 f( \tau ) d\tau \ \ \xrightarrow{\mathcal{L}} \ \ H( s) =\frac{1}{s} F( s)
\end{equation*}
...
f(t) *h(t) =\int\limits ^{t}_0 f( t-\tau ) h( \tau ) d\tau \ \ \xrightarrow{\mathcal{L}} \ \ F( s) H( s)
\end{equation*}
\end{itemize}
...
\begin{equation*}
y(t) =\int ^{t}_0 g( t-\tau ) u( \tau ) d\tau \xrightarrow{\mathcal{L}}\boxed{Y( s) =G( s) U( s)}
\end{equation*}
...
Siano $T_1 ,T_2  >0,\mu  >0,\tau  >0,T_D =T_1  >T_2 ,u(t) =\mathrm{sca}(t)$
\begin{equation*}
...
\begin{equation*}
G( s) =\frac{\beta _2}{s^{2} +\alpha _1 s+\alpha _2} =\mu \frac{\omega ^{2}_n}{s^{2} +2\xi \omega _n s+\omega ^{2}_n}
\end{equation*}
Con $\omega _n  >0$ \textbf{pulsazione naturale} e $\xi $ \textbf{smorzamento}.

La presenza di $\mu $ e $\omega ^{2}_n$ a numeratore permette di lasciare a $\mu $ il ruolo di guadagno:
\begin{equation*}
G( 0) =\mu \frac{\omega ^{2}_n}{\omega ^{2}_n} =\mu 
\end{equation*}
I poli valgono
\begin{equation*}
\begin{aligned}
p_{1,2} & =-\xi \omega _n \pm \sqrt{\xi ^{2} \omega ^{2}_n -\omega ^{2}_n}\\
 & =-\xi \omega _n \pm \omega _n\sqrt{\xi ^{2} -1}
\end{aligned}
\end{equation*}
L'ipotesi affinché siano \textit{complessi} e che ci sia \textit{stabilità} è $0< \xi < 1$
\begin{equation*}
p_{1,2} =-\xi \omega _n \pm i\omega _n\sqrt{1-\xi ^{2}} \ \ \Rightarrow \ \ T=-\frac{1}{\mathrm{Re}( p)} =\frac{1}{\xi \omega _n}
\end{equation*}
Qui $r=2$, allora
\begin{gather*}
y( 0) =0\\
\dot{y}( 0) =0\\
\ddot{y}( 0) =\mu \omega ^{2}_n  >0
\end{gather*}
...
\begin{equation*}
G( s) =\frac{\omega ^{2}_n}{s^{2} +2\xi \omega _n s+\omega ^{2}_n}
\end{equation*}
...
\begin{equation*}
G( i\omega ) =\frac{\omega ^{2}_n}{-\omega ^{2} +i2\xi \omega _n \omega +\omega ^{2}_n} =\frac{1}{\left[ 1-\left(\frac{\omega }{\omega _n}\right)^{2}\right] +i\left[ 2\xi \frac{\omega }{\omega _n}\right]}
\end{equation*}
Scriviamo il modulo
\begin{equation*}
...
Nello studio di queste funzioni di trasferimento è importante mettere in evidenza il termine $\omega ^{2}_n$ a numeratore per valutare correttamente il $\mu $ del sistema, cercando la cosiddetta \textbf{forma standard}, in modo che il termine non influisca prima della sua frequenza caratteristica $\omega _n$
\begin{equation*}
\boxed{G( s) =\frac{\mu }{s^{h}} \cdot \frac{\prod _j( 1+s\tau _j)}{\prod _j( 1+sT_j)} \cdot \frac{\omega ^{2}_n}{s^{2} +2\xi \omega _n s+\omega ^{2}_n}}
\end{equation*}
\textit{Esempio.}
\begin{equation*}
...
\begin{gather*}
\omega ^{2}_n =100\ \ \Rightarrow \ \ \omega _n =10\\
...
\begin{equation*}
G( s) =\frac{\mu }{s^{h}} \cdot \frac{\prod _j( 1+s\tau _j)}{\prod _j( 1+sT_j)} \cdot \frac{s^{2} +2\xi \omega _n s+\omega ^{2}_n}{\omega ^{2}_n}
\end{equation*}
...
\begin{itemize}
\item $\mathrm{Re}( p_i) \leqslant 0\ \forall i$ (poli di $L( s)$)
